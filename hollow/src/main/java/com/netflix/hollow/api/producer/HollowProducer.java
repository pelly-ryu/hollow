...
public class HollowProducer {
    ...
    public HollowProducer(VersionMinter versionMinter, HollowPublisher publisher, HollowAnnouncer announcer) {
        ...
        announced = StateTransition.discontinuous();
    }
    ...
    public void restoreFrom(HollowReadStateEngine priorAnnouncedState, long priorAnnouncedVersion) {
        ...
        announced = StateTransition.restored(priorAnnouncedVersion);
    }
    ...
    private void publish(WriteState writeState) throws IOException {
        ...
        File snapshotFile = publisher.openSnapshot(announced.current);
        File deltaFile = publisher.openDelta(announced.previous, announced.current);
        File reverseDeltaFile = publisher.openReverseDelta(announced.previous, announced.current);

        try(OutputStream os = new BufferedOutputStream(new FileOutputStream(snapshotFile))) {
            writer.writeSnapshot(os);
        }

        if(announced.isBiDirectional()) {
            ...
            publisher.publishDelta(deltaFile, announced.previous, announced.current);
            publisher.publishReverseDelta(reverseDeltaFile, announced.previous, announced.current);
        ...
        try {
            ...
            publisher.publishSnapshot(snapshotFile, announced.current);
            snapshotFile.delete();
